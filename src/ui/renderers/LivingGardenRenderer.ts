...
export const LivingGardenRenderer = {
  render(
    ...
    const ecosystem = visions.map(vision => {
      ...
      const visionMilestones = milestonesToday.filter(m => 
        m.parentId === vision.id && (m.parentLevel ?? "vision") === "vision"
      );
      const visionFocuses = focusesInWeek.filter(f => 
        f.parentId === vision.id && f.parentLevel === "vision"
      );
      const visionIntentions = intentionsInWeek.filter(i => 
        getVisionIdForGoal(i) === vision.id
      );

      // Add milestone-linked focuses and intentions
      visionMilestones.forEach(milestone => {
        const milestoneFocuses = focusesInWeek.filter(f => 
          f.parentId === milestone.id && f.parentLevel === "milestone"
        );
        ...
        const milestoneIntentions = intentionsInWeek.filter(i => {
          ...
          return ancestor?.parentId === milestone.id && ancestor.parentLevel === "milestone";
        });
      ...
      return {
        ...
        isAlive: visionMilestones.length > 0 || visionFocuses.length > 0 || visionIntentions.length > 0
      };
    });
    ...
    if (isMobile) {
      container.className = "garden-view-container living-garden living-garden--mobile";
                    ...
                    <div class="living-garden-plot-growth">
                      ${plot.milestones.length > 0 ? `
                        <div class="living-garden-sprouts">
                          ${plot.milestones.slice(0, 2).map(m => `
                      ...
                      ${plot.intentions.length > 0 ? `
                        <div class="living-garden-leaves">
                          ${plot.intentions.slice(0, 3).map(i => `
                ...
                <div class="living-garden-reflection-body">
                  <div class="living-garden-reflection-question">
                    ...
                    <div class="living-garden-reflection-options">
                      ${["Flourishing 🌸", "Steady growth 🌿", "Some challenges 🍂", "Dormant period ❄️"].map(opt => {
                  ...
                  <div class="living-garden-reflection-question">
                    ...
                    <div class="living-garden-reflection-options">
                      ${["More time ⏰", "Better tools 🛠️", "Different approach 💡", "Rest and recovery 😴"].map(opt => {
      ...
      container.querySelectorAll<HTMLElement>("[data-action='set-reflection']").forEach((btn) => {
        btn.addEventListener("click", (e) => {
          ...
          void WeekReflections.upsert(weekYear, weekNum, nextAnswers).then(() => {
            eventBus.emit("ui:toast", { icon: "🌱", message: "Reflection saved." });
            eventBus.emit("view:changed", { transition: false });
          });
        });
      });
      ...
      container.querySelector<HTMLElement>("[data-action='clear-reflection']")?.addEventListener("click", (e) => {
        ...
        void WeekReflections.clear(weekYear, weekNum).then(() => {
          eventBus.emit("ui:toast", { icon: "🧹", message: "Reflection cleared." });
          eventBus.emit("view:changed", { transition: false });
        });
      });
    }
