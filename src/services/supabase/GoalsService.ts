...
export class GoalsService {
  async getGoals(): Promise<Goal[]> {
    ...
    if (!user) {
      console.warn(
        "[GoalsService] getGoals called without authenticated user; returning empty array",
      );
      return [];
    }
  ...
  async saveGoal(goal: Goal): Promise<void> {
    ...
    if (!user) {
      const error = new AuthenticationError(
        "Cannot save goal: User not authenticated",
      );
    ...
    if (!user.id || typeof user.id !== "string") {
      const error = new AuthenticationError(
        "Invalid user ID: user authentication is invalid",
      );
      console.error("[GoalsService] saveGoal failed:", error.message, {
        userId: user.id,
      });
    ...
    if (typeof goal.month !== "number" || !Number.isFinite(goal.month)) {
      throw new DatabaseError(
        `Invalid month value: ${goal.month}. Month must be a valid number.`,
        null,
      );
    }
    if (typeof goal.year !== "number" || !Number.isFinite(goal.year)) {
      throw new DatabaseError(
        `Invalid year value: ${goal.year}. Year must be a valid number.`,
        null,
      );
    }
    if (goal.year < 1900 || goal.year > 2100) {
      throw new DatabaseError(
        `Invalid year value: ${goal.year}. Year must be between 1900 and 2100.`,
        null,
      );
    }
    if (goal.month < 0 || goal.month > 11) {
      throw new DatabaseError(
        `Invalid month value: ${goal.month}. Month must be between 0 (January) and 11 (December).`,
        null,
      );
    }
    ...
    try {
      ...
      try {
        ...
        if (
          !schemaCheck.success &&
          schemaCheck.missingColumns &&
          schemaCheck.missingColumns.length > 0
        ) {
          console.warn(
            "[GoalsService] Schema verification found missing columns:",
            schemaCheck.missingColumns,
          );
          console.warn(
            "[GoalsService] This may cause save failures. Please run migrations 009_add_goal_meta.sql and 010_add_goal_activity_id.sql",
          );
        }
      } catch (schemaErr) {
        // Don't fail the save if schema check fails, just log it
        console.warn(
          "[GoalsService] Schema verification failed (non-blocking):",
          schemaErr,
        );
      }
      ...
      if (error) {
        ...
        const errorContext = {
          ...
          goalDataSummary: {
            ...
            year: goalData.year,
            meta: goalData.meta
              ? typeof goalData.meta === "object"
                ? Object.keys(goalData.meta)
                : goalData.meta
              : null,
        ...
        throw new DatabaseError(
          `Failed to save goal "${goal.title}": ${userFriendlyMessage}`,
          error,
        );
      }
      ...
      if (!data || data.length === 0) {
        console.warn(
          "[GoalsService] Upsert returned no data - goal may not have been saved:",
          {
            goalId: goal.id,
            goalTitle: goal.title,
            level: goal.level,
        ...
        console.log(
          `✓ Saved goal: "${goal.title}" (${goal.id}) - Database confirmed`,
          {
            level: goal.level,
            returnedData:
              data.length > 0 ? { id: data[0].id, level: data[0].level } : null,
          },
        );
      }
  ...
  async saveGoals(goals: Goal[]): Promise<void> {
    ...
    if (!user) {
      const error = new AuthenticationError(
        "Cannot batch save goals: User not authenticated",
      );
    ...
    try {
      ...
      if (error) {
        ...
        throw new DatabaseError(
          `Failed to batch save goals: ${error.message}`,
          error,
        );
      }
      ...
      if (error) {
        ...
        if (error.message.includes("meta") || error.code === "42703") {
          ...
          if (error.message.includes("activity_id"))
            missingColumns.push("activity_id");
        }
   ...
   */
  async diagnosticGetAllGoals(): Promise<{
    success: boolean;
    goals?: Goal[];
    error?: string;
    count?: number;
    ...
    try {
      ...
      console.log(
        `[GoalsService] Diagnostic: Found ${goals.length} goals in database for user ${user.id}`,
      );
      return {
        success: true,
        goals,
