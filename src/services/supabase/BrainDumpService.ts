...
export class BrainDumpService {
    ...
    try {
      ...
      if (error) {
        ...
        throw new DatabaseError(
          `Failed to load brain dump: ${error.message}`,
          error,
        );
      }
      ...
      console.log(
        `✓ Loaded ${entries.length} brain dump entries from database`,
      );
      return entries;
    } catch (err) {
  ...
  async saveBrainDump(entry: BrainDumpEntry): Promise<void> {
    ...
    if (!user) {
      throw new AuthenticationError(
        "Cannot save brain dump: User not authenticated",
      );
    }

    try {
      const supabase = await getSupabaseClient();
      const { error } = await supabase.from("brain_dump").upsert({
        id: entry.id,
        user_id: user.id,
        text: entry.text,
      ...
      if (error) {
        ...
        throw new DatabaseError(
          `Failed to save brain dump: ${error.message}`,
          error,
        );
      }
  ...
  async deleteBrainDump(id: string): Promise<void> {
    try {
      ...
      if (error) {
        ...
        throw new DatabaseError(
          `Failed to delete brain dump: ${error.message}`,
          error,
        );
      }
  ...
  async saveBrainDumpBatch(entries: BrainDumpEntry[]): Promise<void> {
    ...
    if (!user) {
      throw new AuthenticationError(
        "Cannot save brain dump batch: User not authenticated",
      );
    }
    ...
    try {
      ...
      const { error } = await supabase.from("brain_dump").upsert(entriesData);
      ...
      if (error) {
        ...
        throw new DatabaseError(
          `Failed to save brain dump batch: ${error.message}`,
          error,
        );
      }
      console.log(
        `✓ Batch synced ${entries.length} brain dump entries to cloud`,
      );
    } catch (err) {
